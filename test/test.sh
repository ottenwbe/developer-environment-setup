...
docker run --name=test-fedora --volume="${PWD}":/home/ansible:ro --rm fedora-ansible:${FEDORA_VERSION} /bin/bash -c 'cp -r /home/ansible /tmp/ansible && chmod 755 /tmp/ansible/bootstrap_local.sh && /tmp/ansible/bootstrap_local.sh "$@"' -- /home/ansible/test/docker/inventory.yml '{"users": [{"username": "testuser1"}, {"username": "testuser2"}]}' Fedora "${TAGS}" false false
